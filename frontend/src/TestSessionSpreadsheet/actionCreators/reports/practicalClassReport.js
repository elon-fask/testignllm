...
import { prepareMainTableData, getPracticalCandidateState } from '../../lib/helpers';
import { getPaymentStatusConditionalStyle } from './styles/candidateRowConditionalStyles';

export const preparePracticalClassReportData = rawState => {
  const { testSession } = rawState;

  const [regularCandidateIDs, regularCandidates] = getPracticalCandidateState(rawState);
  ...
  const filename = 'practical_class_report.xlsx';
  const wsName = 'Practical Class Report';
...
const downloadPracticalClassReport = () => (dispatch, getState) => {
  const rawState = getState();
  const spreadsheetData = preparePracticalClassReportData(rawState);
...
export default downloadPracticalClassReport;
